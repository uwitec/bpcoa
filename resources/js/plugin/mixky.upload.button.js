...
Ext.namespace("Mixky.plugins");

Mixky.plugins.UploadButtonPlugin = function(config){
    ...
    Mixky.plugins.UploadButtonPlugin.superclass.constructor.call(this);
    this.initComponent();
}

Ext.extend(Mixky.plugins.UploadButtonPlugin, Ext.util.Observable, {
	button : undefined,
	/**
	 * @cfg {Object} strings
	...
	strings: {
		...
		text_remove: '移除文件',
		text_remove_sure: '您确定要从队列中移除文件么?',
		text_error: 'Error',
		...
		error_queue_exceeded: '一次最多上传{0}个文件.',
		...
		error_size_exceeded: '上传文件最大不允许超过20 MB.',
		error_zero_byte_file: '选中文件大小是0字节.',
		error_invalid_filetype: '选中的文件类型无效.',
		error_file_not_found: '文件没有找到 404.',
	 ...
	 */ 
	file_size_limit: "20MB",		  // Default size limit 20MB
	/**
	 ...
	 */ 
	flash_url: "dependences/swfupload/swfupload.swf",	   // Default url, relative to the page url
	...
	initComponent: function() {
		...
		var em = this.button.el.child('em');
		em.setStyle({
			position: 'relative',
			display: 'block'
		});
	...
	startUpload: function() {
		...
		this.uploadFileGrid = new Ext.grid.GridPanel({
		...
		this.uploadDialog = new Ext.Window({
			...
			title : this.strings.text_upload,
			items : this.uploadFileGrid,
			buttons : [{
	...
	setButtonDisabled : function(disabled){
		var movieEl = this.suo.getMovieElement();
		if(disabled){
...
Mixky.plugins.UploadButton = function(config){
	Ext.apply(this, config);
    Mixky.plugins.UploadButton.superclass.constructor.call(this);
}

Ext.extend(Mixky.plugins.UploadButton, Ext.Button, {
	...
	disable : function(){
		this.uploadPlugin.setButtonDisabled(true);
		Mixky.plugins.UploadButton.superclass.disable.call(this);
	},
	
	enable : function(){
		this.uploadPlugin.setButtonDisabled(false);
		Mixky.plugins.UploadButton.superclass.enable.call(this);
	},
	
	beginUpload : function(){
		this.uploadPlugin.show();
	}
});

Ext.reg('mixkyuploadbutton', Mixky.plugins.UploadButton);