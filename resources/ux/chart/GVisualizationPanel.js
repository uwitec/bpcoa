    ...
    return {
        adapt: function(config) {
            ...
            for (var i = 0; i < cols.length; i++) {
                ...
                //alert(convert[f.type]);
                tbl.addColumn('string', c.label || c, id);
            }
            ...
            for (var i = 0; i < rs.length; i++) {
                for (var j = 0; j < cols.length; j++) {
                	if (j == 0) {
                		//alert(rs[i][cols[j]]);
                		tbl.setValue(i, j, rs[i][cols[j]].v);
                		tbl.setFormattedValue(i, j, rs[i][cols[j]].f);
                	} else {
                		tbl.setValue(i, j, rs[i][cols[j]]);
                	}
                }
            }
            return tbl;
...
Ext.ux.GVisualizationPanel = Ext.extend(Ext.Panel, {
    ...
    onLoadCallback: function() {
        ...
        this.directFn(function(result, e){
        	 var tableCfg = {
        			 datas: [],
