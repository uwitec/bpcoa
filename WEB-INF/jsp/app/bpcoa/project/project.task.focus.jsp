...
Ext.onReady(function(){
	...
	//
	var btnFavoriteOffTask = new Ext.Action({
    	text : '取消关注',
    	iconCls : 'icon-app-mkoa-favoriteoff',
    	handler : function(){
			var node = tree.getSelectionModel().getSelectedNode();
			if(node && node != null && (node.getDepth()==1)){
				BpcProjectAppDirect.focusTask(node.attributes.key,0, function(result, e){
					if((result && result.success == true)){

							MixkyApp.showInfoMessage('任务取消关注成功！');
						}else{
							MixkyApp.showErrorMessage('取消关注失败，请重新操作！');
						}
						
					//}
				});
			}else{
				MixkyApp.showErrorMessage('请选择根任务取消关注！');
			}
    	}
    });
	...
	//
	var btnFavoriteTask = new Ext.Action({
    	text : '关注任务',
    	iconCls : 'icon-app-mkoa-favorite',
    	handler : function(){
			var node = tree.getSelectionModel().getSelectedNode();
			if(node && node != null && (node.getDepth()==1)){
				BpcProjectAppDirect.focusTask(node.attributes.key,1, function(result, e){
					if((result && result.success == true)){

							MixkyApp.showInfoMessage('任务收藏关注成功！');
						}else{
							MixkyApp.showErrorMessage('收藏失败，请重新操作！');
						}
						
					//}
				});
			}else{
				MixkyApp.showErrorMessage('请选择根任务收藏关注！');
			}
    	}
    });
	...
	var tree = new Ext.ux.tree.TreeGrid({
        ...
        contextMenu : new Ext.menu.Menu({items : [btnAddTask, '-', btnEditTask,'-',btnFavoriteTask,btnFavoriteOffTask,'-', btnRefresh]}),
		columns:[{
			header: '标题',
			dataIndex: 'F_TITLE',
			width: 230,
