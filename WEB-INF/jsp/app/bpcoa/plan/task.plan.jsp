...
Ext.onReady(function(){
	...
	var store = new Ext.data.DirectStore({
		...
		directFn : BpcProjectAppDirect.getTaskPlay,
...
%>
	...
	var dateSelect = new Ext.form.DateField({
		...
		listeners : {
			'select' : function(){
				...
				ReloadComboData();
				store.reload();
				worklogPanel.refresh({f_plan_id: 0});
			}
		}
	});
	// 下一月
	...
	// 视图操作
	var buttons = ['-',btnPreMonth, dateSelect, btnNextMonth, '-', lblUser, comboUser, '-', '->', <%=ViewManager.VN_REFRESH_BUTTON_NAME%>, '->'];
	// add by qi end
<%
...
%>
    ...
    var worklogPanel = new Ext.Panel({
		...
		autoLoad : {
			url : 'page.do',
			scripts:true,
			params:{panelid:worklogPanelid, url:'app/bpcoa/plan/plan.log', userid: 0, username: '', date: '2013-01-01'},
			border:false
		},
		width : 650,
		split : true,
	...
	grid.on('rowclick', function(){
		...
		if (record){
			...
			worklogPanel.refresh({f_plan_id:record.get('ID')});
		}
	});
