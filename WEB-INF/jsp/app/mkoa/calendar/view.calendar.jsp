...
Ext.onReady(function(){
    ...
    var count = 0;
    Mixky.app.common.LoadJsFile('mixkyjs-calendar-extend', 'app/mkoa/calendar/calendar.extend.js');
    var fn = function(){
        // 判断js是否加载完毕
        if(!Mixky.calendar.Language && count++ < 100){
            fn.defer(50);
        }else{
            ...
            ds.initialLoad(userId, function(backObj){
                ...
                var mp = new Ext.ux.calendar.MainPanel({
                    datasource:ds,
                    calendarSetting:cs,
                    lang : Mixky.calendar.Language
                });
        		panel.add(mp);
        		panel.doLayout();
            }, Mixky.calendar.Language);
        }
    };        
    fn();
});
