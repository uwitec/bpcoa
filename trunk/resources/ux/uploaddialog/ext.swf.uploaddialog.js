...
Ext.ux.SwfUploadPanel = Ext.extend(Ext.grid.GridPanel, {
	 ...
	 */ 
	file_size_limit: "20MB",		  // Default size limit 100MB
	/**
...
//			width: this.width - 7
		...
		this.addListener({
			...
			// modified by mh
			render: function(){
				this.addBtn = this.getBottomToolbar().items.items[0];
				this.cancelBtn = this.getBottomToolbar().items.items[2];
				this.uploadBtn = this.getBottomToolbar().items.items[3];
				this.clearBtn = this.getBottomToolbar().items.items[4];
...
Ext.ux.SwfUploadDialog = Ext.extend( Ext.Window, {
	...
	constructor: function(config){
		var panelConfig = {
			 frame: false,
			 post_params : {
				"ASPSESSID" : 'mixkynovosky8899998800000000001'
			 },
			 upload_url : config.url
		};
		Ext.applyIf(panelConfig, config);
		panel = new Ext.ux.SwfUploadPanel(panelConfig); 
		Ext.applyIf(panel.post_params, config.base_params);
		...
		Ext.applyIf(config, {
			...
			listeners:{
				//'show': this.initSwfUpload
			}
		});
		Ext.ux.SwfUploadDialog.superclass.constructor.apply(this, [config]);
	},
	initComponent: function() {
		this.width = ( !this.width || this.width < 453 ) ? 453 : this.width;
		this.height = ( !this.height || this.height < 300 ) ? 300 : this.height;
		Ext.ux.SwfUploadDialog.superclass.initComponent.apply( this, arguments );
	}
});