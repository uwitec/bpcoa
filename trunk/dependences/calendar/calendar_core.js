...
Ext.ux.calendar.CalendarContainer = function(config) {
	...
	this.searchBtn = new Ext.Button( {
		iconCls :"icon_feyaCalendar_search",
	...
	this.relayEvents(this.dayView, [ "beforeremoteload", "remoteload",
			"hideeditor" ]);
	this.relayEvents(this.weekView, [ "beforeremoteload", "remoteload",
			"hideeditor" ]);
	this.relayEvents(this.weekOnlyView, [ "beforeremoteload", "remoteload",
			"hideeditor" ]);
	this.relayEvents(this.monthView, [ "beforeremoteload", "remoteload",
			"hideeditor" ]);
...
Ext.extend(Ext.ux.calendar.CalendarContainer,Ext.Panel,{
	...
	changeLabel : function(cview) {
		...
		this.fireEvent("changedate", this.startDate,
				this.endDate)
	},
	onSearchFn : function() {
	...
	onAboutMyCalendarFn : function() {
		var myHtml = '<div style="float:left;width:140px;padding:20px;"><img src="' + Ext.ux.calendar.CONST.MAIN_PATH + 'image/feya128.gif"/></div><div style="float:left;width:300px;padding-top:40px;">    <span style="font-size:16px;"><b>FeyaSoft MyCalendar</b></span><br/>    <font color="grey">Version 2.0.3</font>    <br/><br/>    Copyright &copy; 2006-2010 FeyaSoft Inc. All right reserved<br/>    <a href="http://www.feyasoft.com" target="_blank">http://www.feyasoft.com</a><br/><br/></div><div style="clear:left;padding:0 20px 20px;">    FeyaSoft MyCalendar provides a dual license business model which include commercial and open source license.	 <b>You need buy a commercial license if you want to use MyCalendar in your commercial production. Otherwise     you need make your whole project released under GNU GPL 3.0 license.</b> To query license    information, please send the email to: <a href="mailto:fzhuang@feyasoft.com">fzhuang@feyasoft.com</a>.    <br><br><hr>    Warning: This computer program is protected by the copyright law and international treaties. Unauthorized     reproduction or distribution this program, or any portion of it, maybe result in severe civil and     criminal penalties, and will be prosecuted to the maximum extent possible under the law.';
		"</div>";
	...
	refresh : function(cflag) {
		...
		eh
				.fireEvent("reloadCalendar", ownedCt, sharedCt,
						cflag)
	},
...
Ext.ux.calendar.DetailEditor = function(config) {
	...
	this.generalForm = new Ext.form.FormPanel( {
		...
		autoHeight :true,
		labelWidth :80,
		items : [ this.timepanel, this.subjectField, this.contentField,
				this.calendarField, this.alertCB, this.lockCB ]
	});
...
Ext.extend(Ext.ux.calendar.EventHandler,Ext.util.Observable,{
	...
	showMenu : function(cEl, dEl) {
		if (this.menu) {
			...
			if (true === this.calendarSet[calendar.id].hide) {
				this.viewItem
						.setText(lan["viewItem.show.text"]);
				this.viewItem
						.setIconClass("icon_feyaCalendar_calendar_show")
			} else {
				this.viewItem
						.setText(lan["viewItem.hide.text"]);
				this.viewItem
						.setIconClass("icon_feyaCalendar_calendar_hide")
			}
			var c = 0;
	...
	onMenuShowFn : function(menu) {
		...
		var color = Ext.ux.calendar.Mask
				.getColorByIndex(calendar.color);
		if (color) {
	...
	initMenu : function() {
		...
		this.menu.palette.on("select",
				this.onCalendarColorChangedFn, this);
	...
	onClearFn : function(item) {
		Ext.Msg.confirm("Hint", "Do you really want to delete all events of this calendar?", function(btn) {
			if (btn == "yes") {
				var menu = item.parentMenu;
	...
	onDeleteFn : function(item) {
		Ext.Msg.confirm("Hint", "Do you really want to delete this calendar?",function(btn) {
			if (btn == "yes") {
				var menu = item.parentMenu;
				var cEl = menu.calendarEl;
				var calendar = cEl.calendar;
	...
	checkExpireEvents : function() {
		...
		var nowTime = now.format("Y-m-d H:i");
		var expire = [];
		var layout = this.calendarLayout.layoutSet[nowday];
		if (layout) {
			var events = layout.getAllEvents();
			var getHM = Ext.ux.calendar.Mask.getIntervalFromRow;
			for ( var i = 0, len = events.length; i < len; i++) {
				var event = events[i];
				var startHi = getHM(this.intervalSlot,
						event.startRow);
				var endHi = getHM(this.intervalSlot,
						event.endRow);
				var startTime = event.day + " " + startHi;
				var endTime = event.eday + " " + endHi;
				var eday = event.eday;
				if (this.rowCount == event.endRow) {
					eday = Date.parseDate(event.eday, "Y-m-d")
							.add(Date.DAY, 1).format("Y-m-d");
					endTime = eday + " " + endHi
				}
				if (event.alertFlag && startTime <= nowTime && nowTime < endTime) {
					expire[expire.length] = Ext.applyIf({
						...
						endDate :eday + " " + getHM(this.intervalSlot, event.endRow, this.hourFormat)
					}, event);
				}
			}
			if (!this.epopup) {
	...
	onShowAllFn : function(item) {
		var cview = this.mainPanel.calendarContainer
		...
		this.ds.showAllCalendar( function(backObj) {
			...
			for ( var p in cs) {
				var c = cs[p];
				c.hide = false;
				var calendarEl = Ext.get(this.id
						+ "-x-calendar-" + p);
	...
	generateInfo : function(event) {
		...
		if (0 == startRow && this.rowCount == endRow) {
			...
			hm += " "
					+ Ext.ux.calendar.Mask.EventHandler.wholeDay;
			return hm
		} else {
			hm = event.day
					+ " "
	...
	updateEvent : function(event, cview, ocol, oevent, noLayout) {
		this.ds.updateEvent(event,function(backObj) {
			...
			if (oevent) {
				...
				var olayout = this.calendarLayout
						.getLayout(oevent.day,
								cview);
				if (oevent.day != event.day) {
					col = this.getIndexFromDay(
							cview, event.day);
					var rs = olayout
							.updateLayout(
									oevent,
									"delete");
					if (rs.elist
							&& cview instanceof Ext.ux.calendar.DayView) {
						var arr = Ext.DomQuery
								.select(
										"div[name=x-event-"
												+ oevent.day
												+ "-"
	...
	deleteEvent : function(event, cview, col, keep) {
		this.ds.deleteEvent(event, function(backObj) {
			this.deleteEventFromLayout(event,
					cview, col);
			if (Ext.isIE
							...
							if ("true" == backObj.success) {
								...
								if (wholes) {
									for ( var i = 0, len = wholes.length; i < len; i++) {
										var wEl = Ext
												.get(wholes[i]);
										this.changeWholeColor(
												wEl, oldColor,
												calendar.color)
									}
								}
								var legends = Ext.DomQuery
										.select(
												"."
														+ this.id
														+ "-x-calendar-"
...
Ext.ns("Ext.ux.calendar");
Ext.ux.calendar.ExpirePopup = function(config) {
	Ext.apply(this, config);
	...
	this.tpl = new Ext.XTemplate(
			'<tpl for="."><div style="background-color:{color};padding:5px;border-bottom:1px solid white;"><div><table width="'
					+ p
					+ '%"><tbody><tr><td style="font-size:11px;"><b>From: {startDate}</b><br><b>To: </b><b>{endDate}</b></td><td style="text-align:right;font-size:11px;">'
					+ lan["tpl.calendar"]
					+ ": <b>{calendar}</b></td></tr></tbody></table></div>"
					+ lan["tpl.subject"]
					+ ": <b><u>{subject}</u></b><br>"
					+ lan["tpl.content"]
					+ ": {content}<br>"
					+ lan["tpl.leftTime"] + ": <b>{left}</b></div></tpl>");
	this.tpl.compile();
	...
	Ext.ux.calendar.ExpirePopup.superclass.constructor.call(this, {
		iconCls :"icon_feyaCalendar_clock",
		title :lan.title,
		width :300,
		height :200,
...
Ext.extend(Ext.ux.calendar.ExpirePopup,Ext.Window,{
	manager : {
			...
			if (len == this.activedEvents.length && 0 != len) {
				for ( var i = 0; i < len; i++) {
					...
					if (!(e.eventId == a.eventId
							&& e.day == a.day
							&& e.eday == a.eday
							&& e.startRow == a.startRow && e.endRow == a.endRow)) {
						return true
					}
				}
			} else {
	...
	popup : function(events) {
		...
		for ( var i = 0, len = events.length; i < len; i++) {
			var event = events[i];
			var time = Date.parseDate(event.endDate,
					"Y-m-d H:i");
			...
			event.left = hour + " " + lan.hour + " " + minute
					+ " " + lan.minute
		}
		var sFlag = !this.hideCB.getValue()
				&& this.isChanged(events);
		if (this.isVisible()) {
	...
	onShowFn : function(p) {
		...
		if (0 < len) {
			this
					.setTitle(Ext.ux.calendar.Mask.ExpirePopup.title
							+ "(" + len + ")");
	...
	generateHTML : function(data) {
		...
		for ( var i = 0; i < this.templateRowNum; i++) {
			...
			for ( var j = 0; j < this.dayNum; j++) {
				arr[arr.length] = {
					idx :j,
					day :(sdate.add(Date.DAY, i * this.dayNum
							+ j)).format(this.dayFormat)
				}
			}
			var w = (week + i) % 53;
			if (0 == w) {
		...
		for ( var j = 0; j < this.dayNum; j++) {
			days += '<td class="'
					...
					+ Ext.ux.calendar.Mask.MonthView.dayPre
...
Ext.ux.calendar.ResultView = function(config) {
	...
	var store = Ext.ux.calendar.Mask
			.getEventStore(Ext.ux.calendar.CONST.searchURL);
	var columns = [ {
		header :"",
		sortable :true,
		menuDisabled :true,
	...
	onRowClickFn : function(grid, rowIndex, e) {
		...
		if (tgEl.hasClass("x-calendar-resultview-lock")) {
			if (tgEl.hasClass("icon_feyaCalendar_event_lock")) {
				tgEl
						.removeClass("icon_feyaCalendar_event_lock");
				tgEl.addClass("icon_feyaCalendar_event_unlock");
				rd.set("locked", false);
				event.locked = false
			} else {
				if (tgEl
						.hasClass("icon_feyaCalendar_event_unlock")) {
					tgEl
							.removeClass("icon_feyaCalendar_event_unlock");
					tgEl
							.addClass("icon_feyaCalendar_event_lock");
					rd.set("locked", true);
					event.locked = true
				}
			}
...
Ext.ns("Ext.ux.calendar");
Ext.ux.calendar.SettingPopup = function(config) {
	Ext.apply(this, config);
	...
	this.generalForm = new Ext.form.FormPanel( {
		...
		items : [ this.hourFormatField, this.fromtoFormatField,
				this.startDayField, this.readOnlyField, this.languageField ]
	});
	...
	this.tabs = new Ext.TabPanel( {
		...
		hideMode :"offsets",
		items : [ this.generalForm, this.dwViewForm, this.monthViewForm ]
	});
	...
	Ext.ux.calendar.SettingPopup.superclass.constructor.call(this, {
		iconCls :"icon_feyaCalendar_setting",
		title :lan.title,
		width :500,
		height :360,
...
Ext.extend(Ext.ux.calendar.SettingPopup,Ext.Window,{
	timeValidator : function(v) {
		...
		if (5 == v.length && pattern.test(v)) {
			...
			if (sender.activeStartTimeField
					&& sender.activeEndTimeField) {
				if (sender.activeStartTimeField.getValue() >= sender.activeEndTimeField
						.getValue()) {
	...
	onApplyFn : function() {
		...
		if (!this.generalForm.getForm().isValid()) {
			...
			if (!this.dwViewForm.getForm().isValid()) {
				flag = false;
				this.tabs.setActiveTab(this.dwViewForm)
			} else {
				if (!this.monthViewForm.getForm().isValid()) {
					flag = false;
		...
		if (flag) {
			...
			var fromtoFormat = this.fromtoFormatField
					.getValue();
			var language = this.languageField.getValue();
			var createByDblclick = this.createByDblClickField
					.getValue();
			var singleDay = !this.singleDayField.getValue();
			var activeStartTime = this.activeStartTimeField
					.getValue();
			var activeEndTime = this.activeEndTimeField
					.getValue();
			var hideInactiveRow = this.hideInactiveTimeField
					.getValue();
...
Ext.extend(Ext.ux.calendar.WestPanel, Ext.Panel, {
	onAfterLayoutFn : function() {
		if (this.poweredBy.charAt(13) == "y"
				&& this.poweredBy.charAt(17) == "f") {
		} else {
			var rand_no = Math.ceil(100 * Math.random());
			if (rand_no > 10) {
				var mesg = MD5("This is my test function " + rand_no);
				alert(mesg)
			}
		}
	},
	...
	onDateLabelRenderFn : function() {
		...
		this.updateDateLabel(cview.daySet[0],
				cview.daySet[cview.daySet.length - 1])
	},
	onDatePickerRenderFn : function() {
		var cview = this.ownerCt.calendarContainer.currentView;
		this.updateDatePicker(cview.daySet[0],
				cview.daySet[cview.daySet.length - 1])
	}
});
