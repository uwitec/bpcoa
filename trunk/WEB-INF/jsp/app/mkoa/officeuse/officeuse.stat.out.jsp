...
<%
	...
	// 办公用品台账
	List<OfficeUse> uses = OfficeUseManager.instance().getStatOfficeUse(year, "办公用品出库单");
	// 所有考评数据
	List<List<String>> useStats = OfficeUseManager.instance().getMonthOfficeUseStat(year, "办公用品出库单");
...
%>
						<A href='#' onclick="javascript:Ext.getCmp('<%=panelid%>').refresh({year:<%=years.get(i).getAsString()%>, isOut:true})"><%=years.get(i).getAsString()%></A>
<%
	}
%>
						]　　
...
<TABLE class='mixky-report-table' cellspacing='1' cellpadding='0'>
	<TR align='center'>
		...
		<TD class='mixky-report-colheader' nowrap colspan=12>出库数量(发生/有效)</TD>
...
<%
		int useload = 0;
		for(int j=1;j<13;j++){
			String ym = year + "-" + (j > 10 ? "" : "0") + String.valueOf(j);
